...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $updated_password = ($_POST['passwords']);
...
if ($_POST["passwords"] !== $_POST["repeatpassword"]) {
    die("Passwords must match");
        ...
        $hashed = password_hash($updated_password, PASSWORD_DEFAULT);
        $update = $mysqli->prepare("UPDATE user SET password_hash = ? WHERE id = ?");
        $update->bind_param("si", $hashed, $user_id);
        $update->execute();


        header("Location: account-settings.php?passwordupdated=1");
        exit;
}
}
