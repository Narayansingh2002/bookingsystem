...
$sql = "
    SELECT b.id, b.bookingdatetime, b.additionalinformation,
       s.servicename, u.id AS user_id, u.name AS customer_name
    FROM bookings b
...
<main class="content" id="content">
    ...
    <h1 class="services-title">Order History</h1>
    <table id="services">
        <tr>
            <th>Customer</th>
            <th>Service</th>
        ...
        <tr>
            <td><a href="view-profile.php?id=<?= $row['user_id'] ?>"><?= htmlspecialchars($row['customer_name']) ?></a></td>
            <td><?= htmlspecialchars($row['servicename']); ?></td>
            <td><?= date("d M Y, H:i", strtotime($row['bookingdatetime'])); ?></td>
            <td><?= htmlspecialchars($row['additionalinformation']); ?></td>
            <td>
                <a href="update-booking.php?id=<?= $row['id'] ?>" class="edit-button">Edit</a>
                <a href="delete-booking-process.php?id=<?= $row['id'] ?>" class="delete-button">Delete</a>
            </td>
        </tr>
        <?php endwhile; ?>
        </table>
